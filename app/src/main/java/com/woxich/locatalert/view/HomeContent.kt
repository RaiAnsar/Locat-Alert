...
import com.woxich.locatalert.model.HomeNavigationItem
...
@Composable
fun HomeContent(stay: Boolean?, viewModel: HomeViewModel = hiltViewModel()) {
    val firebaseUser = viewModel.firebaseAuth.currentUser
    viewModel.changedValueOfStay(stay)
    if(firebaseUser == null) {
        //there is nobody logged in, redirect to login
        viewModel.redirectToLogin()
        return
    }
    if(!viewModel.stay.collectAsState().value){
        viewModel.redirectToGeoActivity()
        return
    }
